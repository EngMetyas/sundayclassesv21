...
export default async function handler(req:any,res:any){
 ...
 try{
  ...
  const actor=rows?.[0];
  if(!actor||!['site_admin','priest','secretary'].includes(actor.role)) return res.status(403).json({error:'ليس لديك صلاحية إنشاء حسابات.'});

  const {full_name,email,password,role='member',sector_id=null,class_id=null,phone=null,birth_date=null,address=null,gender=null,stage=null}=req.body||{};
  ...
  const allowed:Record<string,string[]>={site_admin:['priest','secretary','sector_secretary','servant','member'],priest:['secretary','sector_secretary','servant','member'],secretary:['sector_secretary','servant','member']};
  ...
  try{
   await db('profiles',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({id:user.id,full_name,username:email,email,role,sector_id:finalSector,class_id:role==='member'?class_id:null,phone:phone||null,birth_date:birth_date||null,address:address||null,gender:gender||null,stage:stage||null,active:true})});
