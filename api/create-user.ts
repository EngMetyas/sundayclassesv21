...
export default async function handler(req:any,res:any){
 ...
 try{
  ...
  const rows=await db(`profiles?select=id,role,sector_id&id=eq.${encodeURIComponent(meData.id)}&limit=1`);const actor=rows?.[0];if(!actor||!['site_admin','priest','secretary'].includes(actor.role))return res.status(403).json({error:'ليس لديك صلاحية إنشاء حسابات.'});
  ...
  const allowed:Record<string,string[]>={site_admin:['priest','secretary','sector_secretary','servant','member'],priest:['secretary','sector_secretary','servant','member'],secretary:['sector_secretary','servant','member']};if(!allowed[actor.role]?.includes(role))return res.status(403).json({error:'لا يمكنك إنشاء هذا النوع من الحسابات.'});
  ...
  const finalSector=sector_id||classRow?.sector_id||null;const email=internalEmail(username);
