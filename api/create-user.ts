...
export default async function handler(req:any,res:any){
 ...
 try{
  ...
  // Normalize stage defensively on the server so Arabic Excel/UI labels can never reach
  // the profiles.stage CHECK constraint as invalid values.
  const stageMap:Record<string,string>={
    'بيبي':'baby','baby':'baby',
    'ابتدائي':'primary','primary':'primary',
    'إعدادي':'prep','اعدادي':'prep','prep':'prep',
    'ثانوي':'secondary','secondary':'secondary',
    'شباب':'youth','youth':'youth'
  };
  const normalizedStage=stage==null||stage===''?null:(stageMap[String(stage).trim()]||String(stage).trim());
  ...
  const finalSector=sector_id||classRow?.sector_id||null; if(role==='member'&&normalizedStage){const expected=finalSector==='baby'?'baby':finalSector==='primary'?'primary':finalSector?.includes('prep')?'prep':finalSector?.includes('secondary')?'secondary':finalSector==='youth'||finalSector?.startsWith('youth_')?'youth':null;if(expected&&normalizedStage!==expected)return res.status(400).json({error:'المرحلة لا تتوافق مع قطاع الفصل.'});}const email=internalEmail(username);
  ...
  try{await db('profiles',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({id:user.id,full_name,username,email,role,sector_id:finalSector,class_id:role==='member'?class_id:null,phone:phone||null,birth_date:birth_date||null,address:address||null,gender:gender||null,stage:normalizedStage,active:true})});}
